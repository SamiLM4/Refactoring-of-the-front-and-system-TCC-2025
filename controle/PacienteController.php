...
class PacienteController extends BaseController
{
    ...
    public function store()
    {
     ...
     AND deleted_at IS NULL",
        ...
        if ($limite > 0 && $atual >= $limite) {
            $this->errorResponse(
                "Limite de usuários atingido para o seu plano ({$limite})."
            );
        }
        // ---------------------------
        ...
        try {
            ...
            if (!empty($data['sexo']))
                $pacienteData['sexo'] = $data['sexo'];
            if (!empty($data['endereco']))
                $pacienteData['endereco'] = $data['endereco'];
            if (!empty($data['telefone']))
                $pacienteData['telefone'] = $data['telefone'];
            if (!empty($data['profissao']))
                $pacienteData['profissao'] = $data['profissao'];
            if (!empty($data['estado_civil']))
                $pacienteData['estado_civil'] = $data['estado_civil'];
            if (!empty($data['nome_cuidador']))
                $pacienteData['nome_cuidador'] = $data['nome_cuidador'];
            if (!empty($data['telefone_cuidador']))
                $pacienteData['telefone_cuidador'] = $data['telefone_cuidador'];
    ...
    public function update($id)
    {
        ...
        try {
            ...
            $pacienteData = [];
            if (isset($data['nome']))
                $pacienteData['nome'] = $data['nome'];
            if (isset($data['cpf']))
                $pacienteData['cpf'] = preg_replace('/[^0-9]/', '', $data['cpf']);
            if (isset($data['sexo']))
                $pacienteData['sexo'] = $data['sexo'];
            ...
            if (isset($data['estado_civil']))
                $pacienteData['estado_civil'] = $data['estado_civil'];
            if (isset($data['nome_cuidador']))
                $pacienteData['nome_cuidador'] = $data['nome_cuidador'];
            if (isset($data['telefone_cuidador']))
                $pacienteData['telefone_cuidador'] = $data['telefone_cuidador'];
            ...
            if ($paciente && $paciente['usuario_id']) {
                ...
                if (isset($data['cpf']))
                    $usuarioData['cpf'] = preg_replace('/[^0-9]/', '', $data['cpf']);
                if (isset($data['nome']))
